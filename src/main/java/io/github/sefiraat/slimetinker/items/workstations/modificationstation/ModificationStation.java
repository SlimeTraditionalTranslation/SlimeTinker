...
public class ModificationStation extends MenuBlock {
    ...
    protected void craft(BlockMenu blockMenu, Player player) {
        ...
        // No item dummy!
        if (item == null) {
            player.sendMessage(ThemeUtils.WARNING + GetText.tr("Input a tool/piece of armour into the first slot."));
            return;
        }
        ...
        if (ItemUtils.isTool(item)) {
            ...
            player.sendMessage(ThemeUtils.WARNING + GetText.tr("The item in the first slot isn't a Tinker's tool/armour piece."));
        }
    }
    ...
    private void modTool(BlockMenu blockMenu, Player player, ItemStack item, ItemStack modItem) {
        ...
        if (modItem == null || !Modifications.getModificationListTool().contains(StackUtils.getIdOrType(modItem))) {
            player.sendMessage(ThemeUtils.WARNING + GetText.tr("Input a valid modifier into the second slot."));
            return;
        }
        ...
        if (!mod.getRequirementMap().containsKey(currentLevel + 1)) { // Max level
            player.sendMessage(ThemeUtils.WARNING + GetText.tr("You have already maxed out this modifier"));
            return;
        }
        ...
        if (currentAmount <= 0) {
            if (modSlots == 0) { // New mod and no slots
                player.sendMessage(ThemeUtils.WARNING + GetText.tr("You do not have enough free Modification slots for this"));
                return;
        ...
        if (!blockMenu.fits(newTool, OUTPUT_SLOT)) {
            player.sendMessage(ThemeUtils.WARNING + GetText.tr("Clear the output slot first"));
            return;
        }
