...
public class ComponentMaterial {
    ...
    public void registerParts() {
        ...
        if (cmToolMakeup.isValidHead()) {
            ...
            new PartTemplate(Categories.PART_DICT, headStack(cmIdentity.getId(), "劍", SkullTextures.PART_SWORD_BLADE), DummySmeltery.TYPE, basicRecipe(Casts.CAST_SWORDBLADE, getLiquidItemStack(2)), cmIdentity.getId()).register(SlimeTinker.inst());
        }
        if (cmToolMakeup.isValidHead()) {
            //CMManager.getMAP_CAST_HOEHEAD().put(this, Parts.HOE_HEAD.getStack(cmIdentity.getId(), HEAD, HOE, getColor()));
            new PartTemplate(Categories.PART_DICT, headStack(cmIdentity.getId(), "鋤頭", SkullTextures.PART_HOE_HEAD), DummySmeltery.TYPE, basicRecipe(Casts.CAST_HOEHEAD, getLiquidItemStack(1)), cmIdentity.getId()).register(SlimeTinker.inst());
        }
        if (cmToolMakeup.isValidHead()) {
            //CMManager.getMAP_CAST_AXEHEAD().put(this, Parts.AXE_HEAD.getStack(cmIdentity.getId(), HEAD, AXE, getColor()));
            new PartTemplate(Categories.PART_DICT, headStack(cmIdentity.getId(), "斧頭", SkullTextures.PART_AXE_HEAD), DummySmeltery.TYPE, basicRecipe(Casts.CAST_AXEHEAD, getLiquidItemStack(1)), cmIdentity.getId()).register(SlimeTinker.inst());
        }
        if (cmToolMakeup.isValidHead()) {
            //CMManager.getMAP_CAST_PICKAXEHEAD().put(this, Parts.PICKAXE_HEAD.getStack(cmIdentity.getId(), HEAD, PICKAXE, getColor()));
            new PartTemplate(Categories.PART_DICT, headStack(cmIdentity.getId(), "鎬子", SkullTextures.PART_PICKAXE_HEAD), DummySmeltery.TYPE, basicRecipe(Casts.CAST_PICKAXEHEAD, getLiquidItemStack(1)), cmIdentity.getId()).register(SlimeTinker.inst());
        }
        if (cmToolMakeup.isValidHead()) {
            //CMManager.getMAP_CAST_SHOVELHEAD().put(this, Parts.SHOVEL_HEAD.getStack(cmIdentity.getId(), HEAD, SHOVEL, getColor()));
            new PartTemplate(Categories.PART_DICT, headStack(cmIdentity.getId(), "鏟子", SkullTextures.PART_SHOVEL_HEAD), DummySmeltery.TYPE, basicRecipe(Casts.CAST_SHOVELHEAD, getLiquidItemStack(1)), cmIdentity.getId()).register(SlimeTinker.inst());
        }
